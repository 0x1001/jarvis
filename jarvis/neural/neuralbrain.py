...
class NeuralBrainException(BrainException): pass

class NeuralBrain(Brain):
    """
        Brain class based on neural network
    ...
    def configure(self,input=0,output=0):
        """
        ...
        if not isinstance(input,int) or not isinstance(output,int):
            raise NeuralBrainException("Bad input. Three int expected.")
